...
type Client struct {
	...
	// Old OAuth1 fields. Оставлены временно для совместимости структуры проекта,
	// но новый Garmin auth их больше не использует.
	oauthToken  string
	oauthSecret string
	...
	// New Garmin DI OAuth2 fields.
	diAccessToken  string
	...
	var data struct {
		DailyWeightSummaries []struct {
			AllWeightMetrics []struct {
				...
				BodyFat        float32 `json:"bodyFat"`
				BodyWater      float32 `json:"bodyWater"`
				BoneMass       float32 `json:"boneMass"`
				MuscleMass     float32 `json:"muscleMass"`
				PhysiqueRating int     `json:"physiqueRating"`
