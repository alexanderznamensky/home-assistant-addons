...
func NewClient() *Client {
	...
	return &Client{
		// Важно иметь Cookie Jar для сессии SSO и API
...
// GetWeight - start и end формата: YYYY-MM-DD (например: 2025-07-28)
func (c *Client) GetWeight(start, end string) ([]*core.Weight, error) {
	...
	for _, day := range data.DailyWeightSummaries {
		for _, metric := range day.AllWeightMetrics {
			...
			// важен для функции DeleteWeight
			...
			w := &core.Weight{
				...
				MetabolicAge:   int(metric.MetabolicAge / 31536e6), // мс в год
...
func (c *Client) AddWeights(weights []*core.Weight) error {
	// Загрузка новых измерений не зависит от наличия weightID,
	// но оставим поведение как в исходнике.
	if len(c.weightID) == 0 {
		return nil
	}
	...
	for len(weights) != 0 {
		...
		// Garmin может падать на больших батчах — режем до 200 записей
		if len(weights) > 200 {
			chunk = weights[:200]
			weights = weights[200:]
		} else {
